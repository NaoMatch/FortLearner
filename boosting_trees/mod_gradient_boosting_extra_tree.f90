module mod_gradient_boosting_extra_tree
    use mod_const
    use mod_common
    use mod_timer
    use mod_stats
    ...
    contains
        ...
        procedure :: dump => dump_gradient_boosting_extra_tree_regressor
        procedure :: load => load_gradient_boosting_extra_tree_regressor
...
contains
    ...
    subroutine dump_gradient_boosting_extra_tree_regressor(this, file_name)
        implicit none
        class(gradient_boosting_extra_tree_regressor) :: this
        ...
        do i=1, size(this%etrees(:)), 1
            call this%etrees(i)%dump_base_tree(newunit)
        end do
        close(newunit)
    end subroutine dump_gradient_boosting_extra_tree_regressor


    !> A subroutine to load trained model.
    !! \param file_name load file name.
    subroutine load_gradient_boosting_extra_tree_regressor(this, file_name)
        implicit none
        class(gradient_boosting_extra_tree_regressor) :: this
        character(len=*), intent(in) :: file_name
        integer(kind=8)              :: newunit, i, n_outputs
        open(newunit=newunit, file=file_name, form="unformatted")
        read(newunit) this%n_estimators_; allocate(this%etrees(this%n_estimators_))
        read(newunit) n_outputs;    this%n_outputs_ = n_outputs; allocate(this%y_train_mean(n_outputs))
        read(newunit) this%y_train_mean(:)
        do i=1, size(this%etrees(:)), 1
            call this%etrees(i)%load_base_tree(newunit)
        end do
        close(newunit)
    end subroutine load_gradient_boosting_extra_tree_regressor




end module mod_gradient_boosting_extra_tree