program main_decision_tree_oblivious_tree
    use mod_timer
    use mod_decision_tree
    use mod_sliq
    use mod_oblivious_tree
    use mod_clouds
    use mod_metric
    use mod_data_holder
    implicit none
    ...
    type(clouds_regressor) :: cloudsreg
    type(sliq_regressor) :: sliqreg
    type(decision_tree_regressor) :: dtreg
    type(oblivious_tree_regressor) :: obtree
    type(metrics)                 :: metric
    ...
    y_train_pred = obtree%predict(x_train)
    print*, metric%mean_square_error(y_train(:,1), y_train_pred(:,1))
    call obtree%dump(file_name="et.bin")
    ...
    print*, metric%mean_square_error(y_train(:,1), y_train_pred(:,1))
    call dtreg%dump(file_name="et.bin")
    ...
    call date_and_time(values=date_value2)
    print*, time_diff(date_value1, date_value2)
    y_train_pred = sliqreg%predict(x_train)
    print*, metric%mean_square_error(y_train(:,1), y_train_pred(:,1))
    call sliqreg%dump(file_name="et.bin")
    ...
    call date_and_time(values=date_value2)
    print*, time_diff(date_value1, date_value2)
    y_train_pred = cloudsreg%predict(x_train)
    print*, metric%mean_square_error(y_train(:,1), y_train_pred(:,1))
    call cloudsreg%dump(file_name="et.bin")

end program main_decision_tree_oblivious_tree