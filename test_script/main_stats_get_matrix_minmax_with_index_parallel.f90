program main_stats_get_matrix_minmax_with_index_parallel
    use mod_timer
    use mod_random
    use mod_stats, only: get_matrix_minmax, get_minmax_hybrid_r8, get_matrix_minmax_parallel
    implicit none
    integer(kind=8) :: date_value1(8), date_value2(8)
    
    integer(kind=8)           :: n_samples, mat_size(5), n_columns, col_size(5), n_indices, idx_size(5), i, j, k, l, n_thds
    ...
    do i=1, 5, 1
        ...
        do k=1, 5, 1
            ...
            call get_matrix_minmax_parallel(min_vals_fast, max_vals_fast, mat_t, idx, & 
                n_indices, n_samples, n_columns, n_thds)
            do l=1, n_columns, 1
                do j=1, n_indices, 1
                    vec(j) = mat_t(l,idx(j))
                end do
...
end program main_stats_get_matrix_minmax_with_index_parallel
