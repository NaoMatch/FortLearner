module mod_check_non_positive
    use :: iso_fortran_env,       only: error_unit
    use :: mod_kinds,             only: r64, i64
    use :: mod_error_codes,       only: ERR_POSITIVE_VALUE, ERR_NON_FINITE
    use :: mod_error_manager,     only: raise_error
    use :: mod_character_helpers, only: make_prefix
    use :: mod_predicates,        only: is_finite, is_non_positive
    use :: mod_program_limits,    only: MAX_MSG_LEN
    implicit none


    interface check_non_positive
        module procedure check_non_positive_r64
        module procedure check_non_positive_i64
    end interface ! check_non_positive

contains

    subroutine check_non_positive_r64(a, file, class_name, value_name, fatal)
        ...
        ! Reject positive numbers	
        else if (.not. is_non_positive(a)) then
            prefix = make_prefix(file, class_name, value_name)
    ...
    end subroutine check_non_positive_r64

    subroutine check_non_positive_i64(a, file, class_name, value_name, fatal)
        ...
        ! Reject positive numbers	
        if (.not. is_non_positive(a)) then
            prefix = make_prefix(file, class_name, value_name)
            write(tmp_msg,'(a," must be <= 0, but got ",i0,".")') prefix, a
            msg = trim(tmp_msg)
            call raise_error(ERR_POSITIVE_VALUE, msg, do_fatal)
        end if
    end subroutine check_non_positive_i64

end module mod_check_non_positive
