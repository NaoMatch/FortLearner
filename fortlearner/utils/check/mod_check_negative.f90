module mod_check_negative
    use :: iso_fortran_env,       only: error_unit
    use :: mod_kinds,             only: r64, i64
    use :: mod_error_codes,       only: ERR_NON_NEGATIVE_VALUE, ERR_NON_FINITE
    use :: mod_error_manager,     only: raise_error
    use :: mod_character_helpers, only: make_prefix
    use :: mod_predicates,        only: is_finite, is_negative
    use :: mod_program_limits,    only: MAX_MSG_LEN
    implicit none

contains

    subroutine check_negative_r64(a, file, class_name, value_name, fatal)
        ...
        ! Reject non-negative numbers
        else if (.not. is_negative(a)) then
            prefix = make_prefix(file, class_name, value_name)
            write(tmp_msg, '(a," must be < 0, but got ",es23.16,".")') prefix, a
            msg = trim(tmp_msg)
            call raise_error(ERR_NON_NEGATIVE_VALUE, msg, do_fatal)
        end if
    end subroutine check_negative_r64

    subroutine check_negative_i64(a, file, class_name, value_name, fatal)
        ...
        ! Reject non-negative numbers
        if (.not. is_negative(a)) then
            prefix = make_prefix(file, class_name, value_name)
            write(tmp_msg, '(a," must be < 0, but got ",i0,".")') prefix, a
            msg = trim(tmp_msg)
            call raise_error(ERR_NON_NEGATIVE_VALUE, msg, do_fatal)
        end if
    end subroutine check_negative_i64

end module mod_check_negative
